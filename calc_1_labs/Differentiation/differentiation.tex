...
\hline
  ...
  \text[12pt]{Constant Rule} & \ddx c=\answer{0}\\
  \text{Constant Multiple Rule} & \begin{prompt}\ddx c\cdot f(x)=\answer{c}\cdot\ddx \answer{f(x)}\end{prompt}\\
  \text{Sum Rule} & \begin{prompt}\ddx (f(x)+g(x))=\ddx \answer{f(x)}+\ddx \answer{g(x)}\end{prompt}\\
  \text{Difference Rule} & \begin{prompt}\ddx (f(x)-g(x))=\ddx \answer{f(x)}-\ddx \answer{g(x)}\end{prompt}
\end{array}
\]
\end{question}
