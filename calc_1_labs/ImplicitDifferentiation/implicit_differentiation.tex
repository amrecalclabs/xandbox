...
\end{onlineOnly}
\begin{question}
What did you get as output from your sage cell? (copy just the answer portion after the "==") $\answer{-1/2*(sin(x*y(x)^2)*y(x)^2 + 2*x*cos(x^2))/(x*sin(x*y(x)^2)*y(x))}$
\end{question}
\begin{question}
...
\end{sageCell}
\end{onlineOnly}
Now use Sage Math again to find $\frac{dy}{dx}$ for  $\sin(x^2)=\cos(xy^2)$.
