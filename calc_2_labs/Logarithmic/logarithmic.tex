...
\choice{log$_b(y)$ - log$_b(x)$}
\choice{$\dfrac{\text{log}_b(x)}{\text{log}_b(y)}$}
\choice[correct]{log$_b(x)$ - log$_b(y)$}
\choice{$\dfrac{\text{log}_b(y)}{\text{log}_b(x)}$}
\end{multipleChoice}

log$_b(x*y)$
\begin{multipleChoice}
\choice{log$_b(y)$ + log$_b(x)$}
\choice{$\text{log}_b(x) \cdot \text{log}_b(y)$}
\choice{$\text{log}_b(y) \cdot \text{log}_b(x)$}
\choice[correct]{log$_b(x)$ + log$_b(y)$}
\end{multipleChoice}
...
\choice[correct]{$x*\dfrac{d}{dx}\text{ln}(x)$}
\end{multipleChoice}
\begin{feedback}
